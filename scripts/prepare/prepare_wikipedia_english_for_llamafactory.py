#!/usr/bin/env python3
"""
Prepare English monolingual data from wikimedia/wikipedia for LLaMA-Factory.

This is the fallback high-quality English corpus for synthetic English<->Thai
generation. Default output intentionally matches the path expected by the
generation script:
...
def resolve_text(row: dict[str, Any], keys: list[str], min_chars: int, max_chars: int, min_latin_ratio: float) -> str | None:
    candidates: list[str] = []
    for key in keys:
        value = row.get(key)
...
def load_wikipedia_streaming_dataset(
    load_dataset,
    *,
    repo_id: str,
    config: str,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Wikipedia English -> LLaMA-Factory monolingual JSONL")
    ap.add_argument("--repo-id", default="wikimedia/wikipedia")
    ap.add_argument("--config", default="20231101.en")
    ap.add_argument("--data-files", default="20231101.en/train-*.parquet")
    ap.add_argument("--loader", choices=("auto", "config", "parquet"), default="auto")
    ap.add_argument("--split", default="train")
    ap.add_argument("--limit", type=int, default=250_000, help="Max rows to write; 0 means unlimited.")
    ap.add_argument("--min-chars", type=int, default=80)
    ap.add_argument("--max-chars", type=int, default=1200, help="Truncate long articles to this many chars; 0 disables.")
    ...
    ap.add_argument("--out-dir", type=Path, default=root() / "training" / "data" / "monolingual")
    ap.add_argument("--out-name", default="fineweb2_pt_eng_Latn.jsonl")
    ...
    ap.add_argument("--no-hard-exit", action="store_true")
    args = ap.parse_args()
    ...
    text_keys = args.text_keys or ["text"]
    limit = args.limit if args.limit and args.limit > 0 else None

    load_dataset = import_hf_load_dataset()
    ds = load_wikipedia_streaming_dataset(
        load_dataset,
    ...
    with open(out_path, "w", encoding="utf-8") as fo:
        for row in ds:
            ...
            if not isinstance(row, dict):
                row = dict(row)
            text = resolve_text(row, text_keys, args.min_chars, args.max_chars, args.min_latin_ratio)
            if text is None:
                continue
